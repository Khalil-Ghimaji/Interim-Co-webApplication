...
function update_prix_contrat(int $id_contrat)
{
    ...
    $query = "SELECT SUM(prix_final) AS prix_total FROM prestations WHERE id_contrat = ?";
    $request = $bdd->prepare($query);
...
function notifier_client(int $id_contrat,int $id_agent_drh, string $type_de_notification, string $libelle)
{
    ...
    switch ($type_de_notification) {
        case "succes":
            $message = "Votre contrat $libelle a été traité avec succès.";
            break;
        case "echec":
            supprimer_prestations_contrat($id_contrat);
            $message = "Votre contrat $libelle a été refusé";
            break;
    }
...
function mettre_a_jour_prix($id, $nouveau_prix, $nom_table)
{
    ...
    $query = "UPDATE $nom_table SET prix_final = ? WHERE id = ?";
    $request = $bdd->prepare($query);
...
function inserer_date_prestation($id, $date_debut, $date_fin)
{
    ...
    $query = "UPDATE prestations SET date_deb_finale = ?, date_fin_finale = ? WHERE id = ?";
...
function update_contrat($id_contrat)
{
    $bdd = ConnexionBD::openConnexion();

    $query = "UPDATE contrats SET prix = prix_final WHERE id = ?";
    $request = $bdd->prepare($query);
    $request->execute(array($id_contrat));

    $query = "UPDATE prestations SET prix = prix_final, date_debut = date_deb_finale, date_fin = date_fin_finale WHERE id_contrat = ?";
    $request = $bdd->prepare($query);
    $request->execute(array($id_contrat));
